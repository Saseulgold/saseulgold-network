...
func TestRaw(t *testing.T) {
	c0 := TestSHA256Content{ x: "abcd" }
	c1 := TestSHA256Content{ x: "efgh" }
	c2 := TestSHA256Content{ x: "ijkl" }
	c3 := TestSHA256Content{ x: "mlop" }
	contents0 := []Content{c0, c1, c2, c3}

	c4 := TestSHA256Content{ x: "abcd" }
	c5 := TestSHA256Content{ x: "efgh" }
	c6 := TestSHA256Content{ x: "ijkl" }
	c7 := TestSHA256Content{ x: "mlop-" }
	contents1 := []Content{c4, c5, c6, c7}

	tree0, err := NewTree(contents0)

	if err != nil {
		t.Error("TestRaw failed while making new tree")
	}
	fmt.Println("raw merkle root: ", tree0.MerkleRoot())

	tree1, err := NewTree(contents1)

	if err != nil {
		t.Error("TestRaw failed while making new tree")
	}
	fmt.Println("raw merkle root: ", tree1.MerkleRoot())
}
